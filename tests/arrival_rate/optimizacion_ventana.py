...
LOG_OBJETIVO = "./data/01_raw/arrival_rate/Caso_A/N-1800-1500-900-750_sudden_5000.csv"
...
def objective(trial):
    ...
    # Además de la ventana y el detector, arrival_rate optimiza dos parámetros propios:
    # la frecuencia (resolución del muestreo de llegadas) y la granularidad (agregación).
    ...
    frecuencia_arrival_rate = trial.suggest_categorical('frecuencia_arrival_rate', ['15 min', '30 min', '60 min'])
    granularidad_arrival_rate = trial.suggest_categorical('granularidad_arrival_rate', ['1 hour', '6 hours', '12 hours', '1 day'])
    ...
    frec_val = convertir_a_minutos(frecuencia_arrival_rate)
    gran_val = convertir_a_minutos(granularidad_arrival_rate)
    ...
    with open("./tests/arrival_rate/params_template.yml", "r", encoding="utf-8") as f:
        contenido = f.read()
    ...
    contenido = contenido.replace("__FRECUENCIA_ARRIVAL_RATE__", str(frecuencia_arrival_rate))
    contenido = contenido.replace("__GRANULARIDAD_ARRIVAL_RATE__", str(granularidad_arrival_rate))
    ...
    archivo_config = f"./conf/pruebas/arrival_rate_tests_{trial.number}.yml"
    ...
    comando = [sys.executable, "-m", "tests.arrival_rate.script_arrival_rate",
    ...
    if match:
        ...
        try:
            ...
            print(f"Trial finalizado. F-Score={f_score:.4f} | tamano_ventana={tamano_ventana}, "
                  f"salto_ventana={salto_ventana}, frecuencia={frecuencia_arrival_rate}, "
                  f"granularidad={granularidad_arrival_rate}, n_regresion={n_regresion}, n_confirmacion={n_confirmacion}")
            return f_score
