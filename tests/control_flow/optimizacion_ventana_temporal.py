...
# Log sobre el que se optimiza. Se optimiza UN log a la vez.
LOG_OBJETIVO = "./data/01_raw/control_flow/single/pl-5000-single.csv"
...
def objective(trial):
    ...
    # Escala del control-flow: el proceso de préstamos tiene ~12.5 min/traza (1 evento/min),
    # así que estos rangos cubren ventanas de ~30 a ~350 trazas y saltos de ~1 a ~14 trazas.
    tamano_ventana = trial.suggest_categorical('tamano_ventana', [
        '2 hours', '4 hours', '6 hours', '9 hours', '12 hours', '18 hours',
        '1 day', '2 days', '3 days', '4 days'
    ])

    salto_ventana = trial.suggest_categorical('salto_ventana', [
        '13 min', '30 min', '1 hour', '2 hours', '4 hours', '6 hours'
    ])
    ...
    with open("./tests/control_flow/params_template.yml", "r", encoding="utf-8") as f:
        contenido = f.read()
    ...
    archivo_config = f"./conf/pruebas/control_flow_tests_{trial.number}.yml"
