...
def filtrado_arrival_rate(log: pd.DataFrame, parametros: dict):
    ...
    log_filtrado = log[log['concept:name'] == parametros['primera_tarea']].copy()
    ...
    log_filtrado['time:timestamp'] = log_filtrado['time:timestamp'].dt.floor('s')

    log_filtrado = log_filtrado[['case:concept:name', 'time:timestamp', 'trace_real_index', 'concept:name']]

    if DEBUG:
        log_filtrado.to_csv(f"./logs/arrival_rate_filtrado.csv", index=False)
...
def transformacion_arrival_rate(log: pd.DataFrame, parametros: dict):
    ...
    while True:
        ...
        #if DEBUG:
        #    logger.debug(f"Transformando ventana desde {fecha_inicial_str} hasta {fecha_final_str}")
    ...
    if DEBUG:
        log_transformado.to_csv(f"./logs/arrival_rate_transformado.csv", index=False)
...
def preprocesado_original(log: pd.DataFrame, parametros: dict, verbose: bool, test: bool, features: list = None) -> pd.DataFrame:
    """
    ...
    log = log.drop(columns=['time:timestamp','case:concept:name','trace_real_index', 'concept:name'], errors='ignore')

    # Removemos las variables que tiene datos idénticos en todas las filas (varianza cero)
    selector = VarianceThreshold()

    variables_con_varianza = selector.fit_transform(log)
    
    log_preprocesado = pd.DataFrame(variables_con_varianza, columns=log.columns[selector.get_support()])

    if DEBUG:
        log_preprocesado.to_csv(f"./logs/arrival_rate_preprocesado.csv", index=False)
...
def preprocesado_arrival_rate(log: pd.DataFrame, parametros: dict, verbose: bool, test: bool, features: list = None) -> pd.DataFrame:
    ...
    log_prueba.drop(columns=['time:timestamp', 'case:concept:name', 'trace_real_index', 'arrival_rate', 'year', 'month', 'day', 'hour', 'minute', 'week_day', 'es_fin_de_semana' ,'second'], inplace=True, errors='ignore')

    #if DEBUG:
    #    log_prueba.to_csv(f"./logs/arrival_rate_preprocesado_intervalos.csv", index=False)
...
def modelo_arrival_rate(log: pd.DataFrame, parametros: dict, random_state: int = 42) -> dict:
    ...
    #modelo = RandomForestRegressor(criterion="poisson", random_state=random_state)

    log_preprocesado = preprocesado_arrival_rate(log, parametros, verbose=DEBUG, test=False)

    """

    # Filtrar las filas donde arrival_rate es 0 (requerimiento para regresión de Poisson)
    # Las ventanas sin eventos no aportan información significativa al modelo
    log_preprocesado = log_preprocesado[log_preprocesado["arrival_rate"] > 0].copy()
    
    if log_preprocesado.empty:
        logger.warning("No hay ventanas con arrival_rate > 0 después del filtrado. Se requieren datos positivos para la regresión de Poisson.")
        raise ValueError("No hay datos válidos (arrival_rate > 0) para entrenar el modelo de Poisson.")
    
    logger.info(f"Muestras de entrenamiento después de filtrar ceros: {len(log_preprocesado)}")
