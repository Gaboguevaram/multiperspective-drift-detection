...
# Placeholder para futuras transformaciones
...
def inductive_miner(log: pd.DataFrame, parametros: dict) -> dict:
    ...
    Args:
        ...
        parametros: Diccionario con configuración, incluye 'umbral_ruido' para el algoritmo.
    ...
    # Obtener el umbral de ruido de los parámetros.
    umbral_ruido = parametros['umbral_ruido']
    net, initial_marking, final_marking = pm4py.discover_petri_net_inductive(log, noise_threshold=umbral_ruido)

    # Construir el nombre del archivo incluyendo marca de tiempo y versión.
    nombre_archivo = f"./data/06_models/ControlFlow_PetirNet_InductiveMiner_{pd.Timestamp.now().strftime('%Y%m%d_%H%M%S')}"

    logger.info(f"Guardando el modelo descubierto como {nombre_archivo}")
...
def heuristic_miner(log: pd.DataFrame, parametros: dict) -> dict:
    ...
    nombre_archivo = f"./data/06_models/ControlFlow_PetirNet_HeuristicMiner_{pd.Timestamp.now().strftime('%Y%m%d_%H%M%S')}"
