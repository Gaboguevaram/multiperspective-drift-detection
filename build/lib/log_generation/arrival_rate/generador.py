...
def crear_log_con_arrival_rate(TOTAL_CASES_PER_LOG=1250):
    ...
    df_final.rename(columns={
        ...
        'enable_time': 'time:enable_timestamp',
        'start_time': 'time:timestamp',
        'end_time': 'time:end_timestamp'
    }, inplace=True)

    # Se está dejando como columna el tiempo de inicio de una tarea porque en este caso los logs 
    # ejecutan las tareas con recursos infinitos
    # Enable y starts son iguales
    #df_final = df_final[['case:concept:name', 'concept:name', 'time:enable_timestamp', 'time:timestamp', 'time:end_timestamp']]
    df_final = df_final[['case:concept:name', 'concept:name', 'time:timestamp']]
    ...
    df_final.to_csv(f"./log_arrival_rate_{total_trazas}.csv", index=True)
