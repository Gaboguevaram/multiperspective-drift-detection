...
def calcular_salto_equivalente_traza(
    ...
    if tipo_ventana == 'temporal':
        ...
        if total_trazas < 2:
            return None
        # 'time:timestamp' puede llegar aún como string (CSV recién cargado: la conversión
        # a datetime del pipeline ocurre después, en extraccion_ventana_temporal), y sobre
        # strings el .diff() de abajo no puede restar. Se convierte sobre una copia.
        log = log.assign(**{'time:timestamp': pd.to_datetime(log['time:timestamp'], utc=True, errors='coerce')})
