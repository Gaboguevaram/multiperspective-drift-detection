...
def _obtener_calendario(log: pd.DataFrame, parametros: dict) -> pd.DataFrame:
    ...
    El estado de la dependencia con la perspectiva productora 'calendarios' viaja en
    `parametros['calendarios']`. 

    Dos casos:

    1) Ejecución sin productor: no existe la clave 'calendarios' en parametros. 
    ...
    2) Productor acoplado: el orquestador garantiza que el consumer no se lanza hasta
       que el productor 'calendarios' haya producido al menos un modelo. Se drena la
    ...
    """
    estado_dep = parametros.get('calendarios')

    # --- Caso 1: sin productor 'calendarios'. ---
    if estado_dep is None:
        ...
        if getattr(_obtener_calendario, 'calendario_estatico', None) is None:
            logger.info("Sin dependencia 'calendarios' acoplada: descubriendo calendario propio (primera iteración).")
    ...
    while calendarios_pendientes and fecha_inicio_ventana > calendarios_pendientes[0]['validez']:
        logger.info(f"Usando un calendario nuevo desde la cola (fecha validez {calendarios_pendientes[0]['validez']}).")
        calendario = calendarios_pendientes.popleft()['calendario']
