...
def filtrado_service_rate(log: pd.DataFrame, parametros: dict):
    ...
    if DEBUG:
        logger.debug(f"Tarea de fin para filtrado: {parametros['ultima_tarea']}")

    log_filtrado = log[log[DEFAULT_XES_IDS.activity] == parametros['ultima_tarea']].copy()
    ...
    # Eliminar los microsegundos para evitar problemas de precisión en el cálculo de service rate
    ...
    if DEBUG:
        log_filtrado.to_csv(f"./data/02_intermediate/service_rate_{pd.Timestamp.now().strftime('%Y%m%d_%H%M%S_%f')}_filtrado.csv", index=False)

    if log_filtrado.empty:
        logger.warning("El log filtrado para service rate está vacío. No han finalizado trazas para esta ventana")
...
def transformacion_service_rate(log: pd.DataFrame, parametros: dict):
    ...
    if DEBUG:
        log_transformado.to_csv(f"./data/03_primary/service_rate_{pd.Timestamp.now().strftime('%Y%m%d_%H%M%S_%f')}_transformado.csv", index=False)
...
def preprocesado_service_rate(log: pd.DataFrame, parametros: dict, verbose: bool, test: bool, features: list = None) -> pd.DataFrame:
    ...
    if DEBUG:
        log_preprocesado.to_csv(f"./data/05_model_input/service_rate_{pd.Timestamp.now().strftime('%Y%m%d_%H%M%S_%f')}_model_input.csv", index=False)
...
def modelo_service_rate(log: pd.DataFrame, parametros: dict, random_state: int = 42) -> dict:
    ...
    log_preprocesado = preprocesado_service_rate(log, parametros, verbose=DEBUG, test=False)
    ...
    if DEBUG:
        train_X.to_csv(f"./data/04_feature/service_rate_{pd.Timestamp.now().strftime('%Y%m%d_%H%M%S_%f')}_caracteristicas.csv", index=False)
    ...
    ruta_modelo = f"./data/06_models/ServiceRate_{pd.Timestamp.now().strftime('%Y%m%d_%H%M%S_%f')}.joblib"
    joblib.dump(modelo, ruta_modelo)
    logger.info(f"Modelo de service rate guardado en {ruta_modelo}")
