...
def calcular_inverted_support(ventana: pd.DataFrame, modelo: dict, config: dict, metrica: str) -> Dict[str, float]:
    """
    Calcula el soporte inverso por recurso para el modelo de calendarios.

    Devuelve un diccionario {recurso: soporte_inverso}. Recursos ausentes en la
    ...
    Returns:
        Dict[str, float] con el soporte inverso por recurso.
    """
    ...
    for r in recursos:
        ...
        for rc in resource_calendar:
            if rc.resource_name == r:
                ...
                break
        if resource_calendar_r is None:
            logger.warning(f"Recurso {r} de la ventana no encontrado en el calendario: se omite para soporte inverso")
            continue
        inverted_support_r = support_per_resource_inverted(df_r, resource_calendar_r, DEBUG)
        logger.info(f"Recurso: {r}, Soporte inverso: {inverted_support_r:.4f}")
        inverted_por_recurso[r] = inverted_support_r

    if inverted_por_recurso:
        media = float(np.mean(list(inverted_por_recurso.values())))
        logger.info(f"Soporte inverso por recurso ({len(inverted_por_recurso)} recursos), media: {media:.4f}")
    else:
        logger.info("Soporte inverso: ningún recurso del modelo está presente en la ventana.")
...
def comparar_pertenencia_a_distribucion(ventana: pd.DataFrame, modelo: dict, config: dict, metrica: str) -> dict[str, bool]:
    ...
    Args:
        ventana: DataFrame con la ventana de datos.
        modelo: Diccionario con el modelo entrenado (distribuciones por par).
        config: Diccionario con la configuración.
        metrica: Nombre de la métrica a calcular.

    Returns:
        Dict[str, bool] con la pertenencia a la distribución por par (recurso,
    """
...
def comparar_distribuciones(ventana: pd.DataFrame, modelo: dict, config: dict, metrica: str) -> dict[tuple, float]:
    ...
    Returns:
        Dict[tuple, float] con la distancia Wasserstein por par (recurso, tarea).
    """
