...
def deteccion_concept_drift_regresion(parametros: dict, nombre_metrica: str, valor_metrica: float, estado_metrica: dict, traza_mas_nueva: int) -> Tuple[bool, dict, Optional[int]]:
    ...
    # Burn-in post-drift: tras confirmar un drift, el orquestador remina el modelo de
    # referencia y reposiciona la ventana. Durante las primeras n_confirmacion iteraciones
    # la ventana todavía arrastra trazas anteriores al cambio (warm-up): la métrica transita
    # desde el valor de la región de descubrimiento hasta su valor estable, y ese transitorio
    # dispararía una racha de candidatos falsa. Omitimos la detección mientras dura el burn-in
    # (sin acumular esos valores), de modo que la racha solo pueda nacer de un cambio real
    # posterior. Es el equivalente multi-perspectiva del salto i ← i+n del Algoritmo 3.1: se
    # "salta" la región de referencia para la detección, pero SIN mover la ventana compartida
    # (que avanza igual para todas las perspectivas, evitando descoordinarlas).
