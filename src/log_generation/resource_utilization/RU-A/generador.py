...
# <raíz>/data/01_raw/resource_utilization/<CASO>/
# parents[4] sube desde Caso_X/generador_no_drift.py hasta la raíz del repo
# (Caso_X -> resource_utilization -> log_generation -> src -> raíz).
RAIZ      = Path(__file__).resolve().parents[4]
DESTINO   = RAIZ / "data" / "01_raw" / "resource_utilization" / CASO
...
def generar_log_secuencial(TOTAL_CASES: int = 2500, TIPO_CAMBIO: str = "recurring"):
    ...
    output_file = DESTINO / f"./TP_{TIPO_CAMBIO}_{CASO.lower()}_{TOTAL_CASES}-{TIPO_CAMBIO}.csv"
