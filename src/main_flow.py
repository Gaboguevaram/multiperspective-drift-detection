...
def _aplicar_ajuste_ventana(
    ...
    # --- Aplicar el nuevo tamano_ventana y (solo en el inicial) salto_ventana. ---
    # Mutación in-place: parametros_ventana es el dict real del config; el orquestador
    # leerá los nuevos valores en la siguiente extracción/avance.
    ...
    # salto_ventana se calcula UNA SOLA VEZ, durante el ajuste inicial, como el
    # desplazamiento que equivale en promedio a "una traza" en el tipo de ventana
    # actual. En el ajuste post-drift NO se toca: el salto fijado al principio
    # define la unidad de progreso del orquestador, y mantenerlo constante asegura
    # que n_confirmacion siga representando "cuántas trazas para confirmar drift"
    # sin saltos de escala entre fases.
    if momento == 'inicial':
    ...
    # Aseguramos que el detector trabaje con un historial coherente con la nueva
    # ventana del log (n_confirmacion = tamano/salto, n_regresion = n_confirmacion/2).
...
def _procesar_resultados(
    ...
    OJO: las perspectivas que NO dispararon en este tick (modo multi-ventana) no aparecen
    ...
    for resultado in resultados_iteracion:
        ...
        perspectivas_dependientes = [clave for clave, valor in DEPENDENCIAS_ENTRE_PERSPECTIVAS.items() if nombre in valor]
        productor = nombre
...
def _orquestador_uni_ventana(
    ...
    # Estructura: dependencias[consumidor] = {'estado_cambiado_por_dependencia': bool, productor1: {...}, ...}.
    # - 'estado_cambiado_por_dependencia': flag genérico a nivel de consumer. Lo activa la perspectiva consumidora
    #   cuando una de sus dependencias le entrega un modelo nuevo.
    # - Solo se incluyen los productores que están DECLARADOS en esta ejecución. Si un consumer tiene declarada
    #   alguna dependencia en el registro pero su productora no está entre las perspectivas activas, ese productor
    #   se omite y la consumidora opera de forma autónoma para esa dependencia.
        ...
        for p in perspectivas:
            ...
            if cambio_detecado.get(nombre, False):
                ...
                if len(hist_cambios) > 0 and hist_cambios[-1]['iteracion'] == iteracion + 1:
                    ...
                    # Burn-in post-drift (ver deteccion_concept_drift_regresion): tras reminar
                    # el modelo la ventana arrastra trazas pre-drift y la métrica sufre un
                    # transitorio que dispararía una racha falsa. Marcamos las métricas escalares
                    # de ESTA perspectiva para que omitan la detección durante n_confirmacion
                    # iteraciones (el de la ventana ya reajustada). Es per-perspectiva: no toca la
                    # ventana compartida, así que no descoordina al resto de perspectivas.
                    n_conf_burn = p.get('n_confirmacion')
                    if True:
                        if n_conf_burn:
                            for estado_metrica in hist_resultados_perspectivas[nombre].values():
                                if isinstance(estado_metrica, dict) and 'hist_valores' in estado_metrica:
...
def _orquestador_multi_ventana(
    ...
    # Diccionario con las dependencias entre perspectivas, usado para pasar info de una a otra en caso de drift.
    # Estructura: dependencias[consumidor][productor] = {'pendiente': bool, ...claves específicas...}.
    # Las claves específicas del productor (p. ej. 'calendarios_pendientes') las crea `_procesar_dependencias` 
    # tiene un flag genérico 'estado_cambiado_por_dependencia' que se activa cuando una dependencia le da
    # un modelo nuevo.
    # Solo se incluyen los productores que están DECLARADOS en esta ejecución. Si un consumer tiene declarada
    # alguna dependencia en el registro pero su productora no está entre las perspectivas activas, ese productor
    # se omite y la consumidora opera de forma autónoma para esa dependencia.
    ...
    for consumidor in nombres:
        ...
        for productor in listado_productores:
            ...
            if tamano_productor < tamano_consumidor:
                logger.warning(
